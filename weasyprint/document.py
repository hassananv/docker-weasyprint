...
def _gather_links_and_bookmarks(box, bookmarks, links, anchors, interactive_anchors, matrix):
    ...
    has_anchor = anchor_name and anchor_name not in anchors and 'interactive-' not in anchor_name
    has_interactive_anchor = anchor_name and 'interactive-' in anchor_name and anchor_name not in interactive_anchors

    is_attachment = getattr(box, 'is_attachment', False)
    download_name = getattr(box, 'attachment_download', None)

    if has_bookmark or has_link or has_anchor or has_interactive_anchor:
        ...
        if has_interactive_anchor:
            interactive_anchors[anchor_name] = pos_x, pos_y, width, height, box.style['font_size'], box.element            
            

    for child in box.all_children():
        _gather_links_and_bookmarks(child, bookmarks, links, anchors, interactive_anchors, matrix)
...
class Page:
    ...
    def __init__(self, page_box, enable_hinting=False):
        ...
        #: The :obj:`dict` mapping each interactive anchor name to its target
        self.interactive_anchors = {}


        _gather_links_and_bookmarks(
            page_box, self.bookmarks, self.links, self.anchors, self.interactive_anchors, matrix=None)
...
class Document:
    ...
    def resolve_links(self):
        ...
        """
        anchors = set()
        interactive_anchors = set()
        paged_anchors = []
        paged_interactive_anchors = []        
        for i, page in enumerate(self.pages):
            ...
            paged_interactive_anchors.append([])
            for anchor_name, (point_x, point_y, width, height, font_size, element) in page.interactive_anchors.items():
                if anchor_name not in interactive_anchors:
                    paged_interactive_anchors[-1].append((anchor_name, point_x, point_y, width, height, font_size, element))
                    interactive_anchors.add(anchor_name)
        ...
        for page in self.pages:
            ...
            yield page_links, paged_anchors.pop(0), paged_interactive_anchors.pop(0)
    ...
    def write_pdf(self, target=None, zoom=1, attachments=None, finisher=None):
        ...
        for page, links_and_anchors in zip(
            ...
            links, anchors, interactive_anchors = links_and_anchors
            surface.set_size(
                math.floor(scale * (
        ...
        attachment_links = [
            ...
            for page_links, page_anchors, page_interactive_anchors in paged_links_and_anchors]

        interactive_anchors = [page_interactive_anchors          
            for page_links, page_anchors, page_interactive_anchors in paged_links_and_anchors]

        includes_interactive_anchors = False
        for interactive_anchor in interactive_anchors:
            if len(interactive_anchor)>0:
                includes_interactive_anchors = True
                break        
