...
def write_pdf_metadata(fileobj, scale, url_fetcher, attachments,
                       attachment_links, interactive_anchors, pages, finisher):
    ...
    for pdf_page, document_page, page_links, page_interactive_anchors in zip(
            pdf.pages, pages, attachment_links, interactive_anchors):
        ...
        if len(page_interactive_anchors)>0:
            matrix = cairo.Matrix(
                    xx=scale, yy=-scale, y0=document_page.height * scale)
            add_interactive_anchors(page_interactive_anchors, pdf, pdf_page, annotations, matrix )
        elif annotations:
            pdf.extend_dict(pdf_page, pdf_format(
                '/Annots [{0}]', ' '.join(
...
def add_interactive_anchors(page_interactive_anchors, pdf, pdf_page, annotations, matrix):
    acroforms=[]
    
    
    for anchor in page_interactive_anchors:
        rectangle = [anchor[1], anchor[2], anchor[1]+anchor[3], anchor[2]+anchor[4]]
        font_size = anchor[5]

        info = anchor[0].split("-")
        type = info[1]

        attributes = anchor[6].attrib
        # print(attributes)
        init_text = ''
        if 'alt' in attributes:
            init_text = attributes['alt']
        

        color = "0.0 0.0 0.0"
        if 'style' in attributes:            
            color_style = re.search(' color(.+?)\)', attributes['style'])            
            if color_style:
                temp = color_style.group(1).replace('(',' ').replace(')',' ').replace(',',' ')
                color_value = [int(s) for s in temp.split() if s.isdigit()]
                if len(color_value)>=3:
                    color = '{0:f} {1:f} {2:f}'.format(color_value[0]/255.0, color_value[1]/255.0, color_value[2]/255.0)
        ...
        text_ref = pdf.write_new_object(pdf_format(
            '<< /Type /Annot \n'
            '/Subtype /Widget \n'            
            '/FT /Tx \n'
            '/Rect [{0:f} {1:f} {2:f} {3:f}] \n'
            '/T (field_{field_name}) \n'
            '/V ({name}) \n'
            '/DV (Default Name) \n'
            '/DA ({color} rg /F1 {font_size:f} Tf) \n'
            '/F 4 \n'
            '/Ff 4096 \n'
            '>>',
            *rectangle, field_name=anchor[0], name=init_text, color=color, font_size=font_size))
        annotations.append(text_ref)
        acroforms.append(text_ref)
    ...
    if annotations:
        ...
        pdf.extend_dict(pdf.catalog, pdf_format(
            ...
            '  >>', ' '.join('{0} 0 R'.format(n) for n in acroforms)))